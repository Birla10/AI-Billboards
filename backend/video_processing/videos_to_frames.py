...
def extract_frames(video_path, frame_rate=1):
    ...
    try:
        ...
        if not cap.isOpened():
            raise FramesExtractionFailedException(f"Could not open video file {video_path}")
        ...
        raise FramesExtractionFailedException(f"OpenCV error: {str(e)}")
    except Exception as e:
        raise FramesExtractionFailedException(f"Unexpected error: {str(e)}")